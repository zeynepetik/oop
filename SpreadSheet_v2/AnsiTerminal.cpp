...
//#include "utils.h" //FOR GETUSERINPUT()
...
//for user to see what they wrote
std::string AnsiTerminal::getUserInput(int row, int col) {
    this->printAt(row, col, "Enter value: "); 
    std::string userInput;
    char ch;

    while (true) {
        ch = this->getKeystroke(); 
        if (ch == '\n') { 
            break;
        } else if (ch == '\b' || ch == 127) { 
            if (!userInput.empty()) {
                userInput.pop_back(); 
                this->printAt(row, col + 12 + userInput.size(), " "); 
            }
            //MY COMPUTER COULD NOT FIGURE OUT \n THAT'S WHY I INDICATED ANOTHER CHARACTER TO BREAK
        }else if(ch=='^'){
        	break;         
        }else {
            userInput += ch; 
        }
        this->printAt(row, col + 12, userInput); 
    }
